...
docker run -rm=true -v $STATEPATH:/srv/salt:ro -v $PILLARPATH:/srv/pillar:ro salt-states-base/centos:7 salt-call $COMMAND 2>/dev/null | tee log/centos-7.log
...
docker run -rm=true -v $STATEPATH:/srv/salt:ro -v $PILLARPATH:/srv/pillar:ro salt-states-base/ubuntu:14.04 salt-call $COMMAND 2>/dev/null | tee log/ubuntu-14.04.log
