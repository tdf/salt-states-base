...
docker run --rm=true -v $STATEPATH:/srv/salt:ro -v $CONFPATH:/etc/salt/minion.d:ro salt_states_base/$DISTRO bash -c "salt-call --local --force-color saltutil.sync_all; salt-call $COMMAND --local --force-color" | tee /tmp/$$.log
